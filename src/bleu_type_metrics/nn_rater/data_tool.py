...
def readFiles(read_dir, prefix):
    for i, postfix in enumerate([".ref.utr", ".ref.rep", ".hyp.utr", ".hyp.rep"]):
        with open(os.path.join(read_dir, prefix+postfix), "r") as f:
...
def readLabels(read_dir):
    ...
    for prefix in prefixs:
        with open(os.path.join(read_dir, prefix+".label"), "r") as f:
...
def load(read_dir, dict_len=33000, default_w=["<pad>", "<unk>"]):
    ...
    pkl_files = ["train.pkl", "valid.pkl", "dict.pkl"]
    dataset = {}
